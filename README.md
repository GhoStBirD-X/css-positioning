...
Folder ini berisi materi dan contoh terkait "CSS Positioning" (posisi elemen menggunakan CSS). README ini ditulis untuk membantu orang yang baru belajar web development memahami konsep posisi di CSS dengan bahasa yang sopan, jelas, dan praktis.

Siapa yang dituju

- Pemula yang baru belajar HTML dan CSS
- Siswa yang ingin memahami perbedaan tipe posisi
- Siapa saja yang butuh ringkasan cepat beserta contoh
...
- relative
  ...
  - Menggunakan top/right/bottom/left akan memindahkan elemen relatif pada posisinya semula.
...
- absolute
  ...
  - Posisi dihitung relatif terhadap "containing block" (ancestor yang memiliki posisi selain static; biasanya closest parent dengan position: relative/absolute/fixed/sticky). Jika tidak ada ancestor berposisi, posisi dihitung terhadap viewport.
...
Penjelasan singkat: .overlay akan diposisikan relatif terhadap .container karena .container memiliki position: relative. Tanpa posisi pada .container, .overlay akan terikat ke viewport (atau ancestor lain yang berposisi).
...
- Gunakan devtools di browser (Inspect Element) untuk melihat box model, posisi, dan ancestor yang memengaruhi.
...
.parent { position: relative; width: 400px; height: 200px; }
.badge { position: absolute; top: 8px; right: 8px; }

8) Cara melihat hasil di browser

- Buka file HTML yang relevan (mis. index.html) di browser.
- Untuk pengembangan lebih nyaman, gunakan extension Live Server (VS Code) atau jalankan web server lokal sederhana.
